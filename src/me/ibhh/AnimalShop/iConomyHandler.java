...
public class iConomyHandler
{
	...
	private static boolean packageExists(String[] packages) {
		try {
			String[] arrayOfString = packages; int j = packages.length; for (int i = 0; i < j; i++) { String pkg = arrayOfString[i];
			Class.forName(pkg);
			}
			return true;
	...
	public Double getBalance156(Player player)
	{
		...
		if (iConomyversion == 5) {
			...
			try {
				this.balance = new Accounts().get(player.getName()).getHoldings()
						.getBalance();
			} catch (Exception e) {
				System.out.println("[AuctionTrade] No Account! Please report it to an admin!");
			...
			try {
				this.balance = 
						Double.valueOf(Methods.getMethod().getAccount(player.getName())
								.balance());
			} catch (Exception e) {
				System.out.println("[AuctionTrade] No Account! Please report it to an admin!");
